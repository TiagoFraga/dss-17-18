...
public class SGT {
  ...
  public void moveAluno(UnidadeCurricular u, Turno origem, Turno destino, Aluno a) {
    ...
    shift.enroll(a.getMail(), destino.getCodigo(), u.getCodigo());
  ...
  public void mudarProfALecionarTurno(UnidadeCurricular uc, Turno t, Professor p) {
    ...
    ShiftDao shift = new ShiftDao();
    shift.changeShiftTeacher(t.getCodigo(), uc.getCodigo(), p.getMail());
  }
  ...
  public void aceitarTroca(Aluno aluno, Troca troca) {
    ...
    if (troca instanceof PedidoTroca) {
      ...
      trade.deleteTradeByRequest(origem.getMail(), troca.getUcString());
      
      String sOrigem = troca.getOrigem().getCodigo();
      String sDestino = troca.getDestino().getCodigo();
      
      ShiftDao shift = new ShiftDao();
      shift.unenroll(origem.getMail(), troca.getUcString());
      shift.enroll(origem.getMail(), sDestino, troca.getUcString());
      
      shift.unenroll(aluno.getMail(), troca.getUcString());
      shift.enroll(aluno.getMail(), sOrigem, troca.getUcString());
  ...
  public void cancelarTroca(Aluno aluno, Troca troca) {
    ...
    if (troca instanceof PedidoTroca) {
      ...
      TradeDao trade = new TradeDao();
      trade.deleteTradeByRequest(aluno.getMail(), troca.getUcString());
      
    } else {
      u.cancelarInscricaoTroca(aluno, troca);
      TradeDao trade = new TradeDao();
      trade.deleteDirectTrade(aluno.getMail(), troca.getUcString());
  ...
  public boolean inscreverListaTrocas(UnidadeCurricular u, Turno origem, Turno destino, Aluno aluno) {
    ...
    TradeDao trade = new TradeDao();
    trade.addDirectTrade(aluno.getMail(), destino.getCodigo(), u.getCodigo());
    return b;
  }
  ...
  private void carregarUcTroca(TrocaNormal t) {
       ...
       for(Turno destino : lista){
           if(destino.getCodigo().equals((t.getDestinoString()))){
  ...
  private void loadAluno(UnidadeCurricular u, Turno t) {
      ShiftDao shift = new ShiftDao();
      UserDao user = new UserDao();
      ArrayList<String> lista = (ArrayList<String>) shift.getShiftEnrolledStudents(t.getCodigo(), u.getCodigo());
